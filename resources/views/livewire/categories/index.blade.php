        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4" data-reveal>
            @foreach ($categories as $category)
                <a href="{{ route('shop', ['category' => $category->slug]) }}" wire:navigate class="group bg-white rounded-2xl border border-stone-200 p-4 text-center hover:border-brand-300 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300">
                    <div class="relative block w-36 h-36 mx-auto rounded-full overflow-hidden bg-gradient-to-br from-brand-50 to-lime-100 ring-1 ring-stone-100 group-hover:ring-brand-300 transition">
