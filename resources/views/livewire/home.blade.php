<div>
    <section class="md:hidden max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        <img src="{{ asset('storage/heroes/banner.jpg') }}" alt="Sabjify fresh produce banner" class="w-full h-auto block rounded-3xl">
    </section>
...
A pre-designed vegetables and fruits baskets to make sure you don’t miss on your kitchens essential ingredients and also ensure you have your daily nutritions.</p>
    ...
    <section class="bg-stone-50 py-6 sm:py-8">
        ...
        @else
            ...
            <div data-auto-carousel class="lg:hidden relative">
                <div data-carousel-track class="flex gap-4 overflow-x-auto pb-2 snap-x snap-mandatory no-scrollbar">
                @foreach ($this->recipes as $recipe)
                    <a href="{{ route('recipes.show', $recipe) }}" wire:navigate class="group basis-full shrink-0 snap-start bg-white rounded-2xl border border-stone-200 overflow-hidden hover:border-brand-300 hover:shadow-md hover:-translate-y-0.5 transition-all duration-300">
                        <div class="p-3 pb-0">
                        ...
                        <div class="p-4">
                            ...
                            @if ($recipe->description)
                                <p class="mt-1.5 text-xs text-stone-500 leading-relaxed line-clamp-2">{{ $recipe->description }}</p>
                            @endif
                            <p class="mt-2 text-[11px] font-medium text-stone-400">{{ $recipe->products_count }} items</p>
                        </div>
                    </a>
                @endforeach
                <a href="{{ route('recipes.index') }}" wire:navigate class="group basis-full shrink-0 snap-start bg-white rounded-2xl border border-stone-200 overflow-hidden hover:border-brand-300 hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 flex flex-col items-center justify-center p-6">
                    <span class="inline-flex items-center justify-center w-20 h-20 rounded-full bg-gradient-to-br from-brand-50 to-lime-100 ring-1 ring-stone-100 group-hover:ring-brand-300 transition">
                        <i data-lucide="arrow-right" class="w-8 h-8 text-brand-600"></i>
                    </span>
                ...
                <div data-carousel-dots class="mt-3 flex justify-center gap-1.5">
                    @foreach ($this->recipes as $dotRecipe)
                        <button type="button" data-carousel-dot="{{ $loop->index }}" aria-label="Go to recipe {{ $loop->index + 1 }}" class="h-1.5 rounded-full bg-stone-300 transition-all"></button>
                    @endforeach
                    <button type="button" data-carousel-dot="{{ $this->recipes->count() }}" aria-label="Go to view all" class="h-1.5 rounded-full bg-stone-300 transition-all"></button>
                </div>
            </div>
