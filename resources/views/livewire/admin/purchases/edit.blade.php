<div>
    ...
    <div class="bg-white rounded-2xl border border-stone-200 p-6">
        <div class="flex items-center justify-between gap-4 mb-6">
            <h2 class="text-lg font-semibold text-stone-900">Edit Purchase</h2>
            <button type="button" data-confirm-message="Delete purchase {{ $purchaseNumber }}? This will revert stock." @click="$dispatch('confirm-modal', { message: $el.dataset.confirmMessage, action: () => $wire.delete() })" class="inline-flex items-center gap-1.5 rounded-xl border border-red-200 text-red-600 hover:bg-red-50 px-4 py-2 text-sm font-semibold transition">
                <i data-lucide="trash-2" class="w-4 h-4"></i> Delete Purchase
            </button>
        </div>
            ...
            <div>
                ...
                <div class="rounded-xl border border-stone-200 p-4 bg-stone-50/40 mb-4">
                    <div class="grid grid-cols-1 lg:grid-cols-[2fr_110px_110px_120px_auto] gap-3 items-end">
                        ...
                        <div>
                            <label class="block text-[11px] font-medium text-stone-500 mb-1">Qty ({{ $formUnit }}) <span class="text-red-500">*</span></label>
                            <input type="number" wire:model.live="formQty" min="0.001" step="0.001" placeholder="{{ \App\Models\Unit::qtyPlaceholderFor($formUnit) }}" class="w-full rounded-lg border border-stone-300 px-2 py-2 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100" />
                    ...
                    <p class="mt-2 text-xs text-stone-500">
                        ...
                        @endif
                        {{ \App\Models\Unit::qtyHintFor($formUnit) }}
                    </p>
                    @if ($formError)
