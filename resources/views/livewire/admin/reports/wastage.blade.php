<div>
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-lg font-semibold text-stone-900">Wastage Report</h1>
            <p class="text-sm text-stone-500 mt-0.5">Products wasted in the selected period, by reason.</p>
        </div>
    ...
    <div class="grid sm:grid-cols-2 gap-3 mb-6">
        <div class="bg-white rounded-2xl border border-stone-200 p-4">
            <p class="text-xs font-medium text-stone-400 uppercase tracking-wide">Wastage Entries</p>
            <p class="mt-1 text-2xl font-bold text-stone-900">{{ $this->summary['entries'] }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-stone-200 p-4">
            <p class="text-xs font-medium text-stone-400 uppercase tracking-wide">Products Affected</p>
    ...
    <div class="bg-white rounded-2xl border border-stone-200 overflow-hidden">
        <div class="p-4 border-b border-stone-100 flex flex-wrap items-center gap-3">
            ...
            <select wire:model.live="reason" class="rounded-xl border border-stone-300 px-3 py-2 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100 bg-white">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-stone-100">
                    @forelse ($products as $index => $row)
                        <tr class="hover:bg-stone-50" wire:key="wastage-report-{{ $row['product_id'] }}">
                            ...
                            <td class="px-4 py-3 text-right text-stone-700">{{ $row['entries'] }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-12 text-center">
                                ...
                                <p class="mt-3 font-medium text-stone-700">No wastage in this period</p>
