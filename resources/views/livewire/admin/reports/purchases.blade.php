<div>
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-lg font-semibold text-stone-900">Purchase Report</h1>
            <p class="text-sm text-stone-500 mt-0.5">Products purchased from suppliers in the selected period.</p>
        </div>
    ...
    <div class="grid sm:grid-cols-3 gap-3 mb-6">
        <div class="bg-white rounded-2xl border border-stone-200 p-4">
            <p class="text-xs font-medium text-stone-400 uppercase tracking-wide">Total Spent</p>
            <p class="mt-1 text-2xl font-bold text-stone-900">{{ \Illuminate\Support\Number::currency($this->summary['spent'], 'INR') }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-stone-200 p-4">
            <p class="text-xs font-medium text-stone-400 uppercase tracking-wide">Purchase Entries</p>
            <p class="mt-1 text-2xl font-bold text-stone-900">{{ $this->summary['entries'] }}</p>
        </div>
        <div class="bg-white rounded-2xl border border-stone-200 p-4">
            <p class="text-xs font-medium text-stone-400 uppercase tracking-wide">Products</p>
            <p class="mt-1 text-2xl font-bold text-stone-900">{{ $this->summary['products'] }}</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-stone-200 overflow-hidden">
        <div class="p-4 border-b border-stone-100 flex flex-wrap items-center gap-3">
            ...
            <select wire:model.live="supplier" class="rounded-xl border border-stone-300 px-3 py-2 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100 bg-white">
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-stone-100">
                    @forelse ($products as $index => $row)
                        <tr class="hover:bg-stone-50" wire:key="purchase-report-{{ $row['product_id'] }}">
                            ...
                            <td class="px-4 py-3 text-right font-medium text-stone-900">{{ \Illuminate\Support\Number::currency($row['spent'], 'INR') }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-12 text-center">
                                <span class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-stone-100 text-stone-400"><i data-lucide="boxes" class="w-6 h-6"></i></span>
                                <p class="mt-3 font-medium text-stone-700">No purchases in this period</p>
